...
class QubitsEnvironment:
    ...
    def Reward(cls, state, basis):
        ...
        return -1.0 if not cls.Disentangled(state, basis, 10e-4) else 0.0
    ...
    def next_state(self, action, angle=None):
        ...
        if angle is None:
            ...
            self._times_angle_is_optimized += 1
            self._total_angle_iterations += res.nit
            if res.success:
                angle = res.x[0]
            else:
                raise Exception(
                    ...
                    'an error.\n %s' % res.message)
        return U(angle).dot(self._state).astype(np.complex64), angle
    ...
    def step(self, action, angle=None):
        ...
        """
        self._state, _ = self.next_state(action, angle)
        self._last_action = action
        return self._state, self.reward(), self.disentangled()

    def step_ext(self, action, angle=None):
        self._state, angle = self.next_state(action, angle)
        self._last_action = action
        return self._state, angle, self.reward(), self.disentangled()

    def allowed_actions(self):
        last = self._last_action
        res = np.ones(len(self.operators), dtype=np.bool)
        if last is not None:
            res[last] = False
        return res
            # a = self.idx_to_operator[last].split()
            # Single qubit action. Forbid only it
            # if len(a) == 2:
                # res[last] = False
            # Two qubit action.
            # else:
    ...
    def _construct_random_pure_state(self):
        ...
        s = real + 1j * imag
        return (s / np.linalg.norm(s)).astype(np.complex64)
