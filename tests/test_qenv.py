...
from src.quantum_state import (
    VectorQuantumState,
    permute_qubits as permute_qubits_ref,
    phase_norm as phase_norm_ref,
    calculate_q0_q1_entropy_from_rhos as calc_sqe_from_rhos_ref
)
from src.qenv import (
    QEnv as TorchEnv,
    VectorizedQState,
    permute_qubits as permute_qubits_cuda,
    phase_norm as phase_norm_cuda,
    calculate_sqe_from_rhos as calc_sqe_from_rhos_cuda
)
...
@pytest.mark.parametrize(
        "num_qubits,num_envs",
        itertools.product((4,6,8,12), (1,16,32))
)
def test_permute_qubits(num_qubits: int, num_envs: int):
    ...
    a = permute_qubits_ref(x, qubit_indices, num_qubits, inverse=False, inplace=False)
    b = torch.from_numpy(x).to(device="cuda")
    permute_qubits_cuda(b, qubit_indices, inverse=False)
    assert np.all(a == b.cpu().numpy())

    a = permute_qubits_ref(x, qubit_indices, num_qubits, inverse=True, inplace=False)
    b = torch.from_numpy(x).to(device="cuda")
    permute_qubits_cuda(b, qubit_indices, inverse=True)
    assert np.all(a == b.cpu().numpy())
...
def test_phase_norm(num_qubits: int, num_envs: int):
    x = np.array([sample_haar_full(num_qubits) for _ in range(num_envs)])
    z = torch.from_numpy(x).to(device="cuda")
    a = phase_norm_ref(x)
    b = phase_norm_cuda(z)
...
def test_calculate_sqe_from_rhos(num_qubits: int, num_envs: int):
    # Sample states
    ...
    a0, a1 = calc_sqe_from_rhos_ref(rhos)
    b0, b1 = calc_sqe_from_rhos_cuda(torch.from_numpy(rhos).to(device="cuda"))
    assert np.allclose(a0, b0.cpu().numpy())
    assert np.allclose(a1, b1.cpu().numpy())


@pytest.mark.parametrize(
        "num_qubits,num_envs,swaps",
        itertools.product((3,4,6,8,12,16), (1,4,16,32), (False, True))
)
def test_apply(num_qubits: int, num_envs: int, swaps: bool):
    # Sample states
    np.random.seed(7)
    states = np.array([sample_haar_full(num_qubits) for _ in range(num_envs)])

    # Initialize reference & tested quantum state simulators
    numpy_sim = VectorQuantumState(num_qubits, num_envs, "reduced", swaps=swaps)
    torch_sim = VectorizedQState(num_qubits, num_envs, swaps=swaps, device="cpu")

    numpy_sim.states = states
    torch_sim.states = torch.from_numpy(states)
    assert np.allclose(numpy_sim.states, torch_sim.states.numpy())
    ...
    for k in range(20):
        ...
        # --- Test fidelity
        x = fidelity(numpy_sim.states, torch_sim.states.cpu().numpy())
        assert np.all(x > 0.999)


@pytest.mark.parametrize(
        "num_qubits,num_envs",
        itertools.product((3, 4,), (1,))
)
def test_env_step(num_qubits, num_envs):
    np.random.seed(17)
    ...
    assert np.all(numpy_env.simulator.states == torch_env.simulator.states.numpy())
    ...
    for n in range(20):
        ...
        # Test observation fidelity
        x = n_obs.reshape(num_envs, -1)
        y = t_obs.reshape(num_envs, -1).cpu().numpy()
        overlap = fidelity(x, y, True)
        if overlap < 0.9:
            print("\nstep:", n)
            print("\nactions:", actions)
