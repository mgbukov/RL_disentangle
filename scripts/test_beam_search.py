...
from tree_search import beam_search, beam_cost, beam_logcost
...
parser.add_argument('--max_depth', '-d', type=int, default=50, action='store')
parser.add_argument('--beam_size', '-b', type=int, default=32, action='store')
...
for n in range(N):
    ...
    final_state, final_path = beam_search(psi, beam_cost, BEAM_SIZE, MAX_DEPTH)
...
SAVEPATH = f'data/beam-{L}qubits-{MAX_DEPTH}depth/'
...
with open(os.path.join(SAVEPATH, 'output.txt'), mode='w') as f:
    ...
    print(f'Depth limit = {MAX_DEPTH}', file=f)
