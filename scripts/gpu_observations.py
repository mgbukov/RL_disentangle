...
def gpu_rdm_2q_complex(states: np.ndarray):
    ...
    for i, qubits in enumerate(qubit_pairs):
        ...
        psi = torch.permute(statevector, permutation).view(N, 4, -1)
...
if __name__ == "__main__":
    ...
    for B in (64, 128, 256):
        ...
        tic = time.time()
        obs = rdm_2q_complex(states)
        toc = time.time()
        print(f"Elapsed time on CPU for batch size {B}: {toc-tic:.3f}")

        tic = time.time()
        obs = gpu_rdm_2q_complex(states)
        toc = time.time()
        print(f"Elapsed time on GPU for batch size {B}: {toc-tic:.3f}")