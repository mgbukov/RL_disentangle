...
class BeamSearch:
    ...
    class Node:
        ...
        def __init__(self, item, parent, action, cost):
            ...
            @param item (Any): The state to which this node corresponds.
            @param parent (Node): The node in the tree that generated this node.
            @param action (action): The action that was applied to the parent's state to
                generate this node's state.
            @param cost (float): The cost associated with the state of this node.
            """
            self.item = item
            self.parent = parent
            self.action = action
    ...
    def __init__(self, beam_size=1):
        ...
        @param beam_size (int): Maximum size of the beam for the search algorithm.
        """
    ...
    def start(self, psi, env, qubit=None, num_iters=1000, verbose=True):
        ...
        @param psi (numpy.Array): A numpy array of shape (2,2,...,2), giving
            the initial state of the environment.        '--- L ---'
        @param env (QubitsEnvironment): Environment object.
        @param qubit (int):
        # @param qubit (List[int]): A list of ints specifying the indices of the qubits
        #     to be considered as a subsystem. 
