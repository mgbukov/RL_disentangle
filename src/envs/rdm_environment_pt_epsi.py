...
class QubitsEnvironmentV2:
    ...
    def step(self, actions):
        ...
        if self.stochastic:
            # Add noise
            noise = np.random.uniform(0, self.stochastic_eps)
            # noise = self.stochastic_eps
            A = np.random.normal(size=Us.shape) + 1j * np.random.normal(size=Us.shape)
...
def entanglement_of_formation(rhos):
    assert rhos.ndim in (2,3)
    # Compute concurrence
    sigma_y = np.array([[0, -1j], [1j, 0]], dtype=rhos.dtype)
    sigma_yy = np.kron(sigma_y, sigma_y)
    # (4,4) @ (B, 4, 4) @ (4, 4) => (B, 4, 4)
    rhos_tilde = sigma_yy @ np.conj(rhos) @ sigma_yy
    eps = 1e-3 * np.finfo(rhos.dtype).eps * np.eye(4)
    # (B, 4, 4)
    if rhos.ndim == 3:
        sqrt_rhos = np.stack([sqrtm(rho + eps) for rho in rhos])
    else:
        sqrt_rhos = sqrtm(rhos + eps)
    R2 = sqrt_rhos @ rhos_tilde @ sqrt_rhos
    # (B, 4, 4)
    if rhos.ndim == 3:
        R = np.stack([sqrtm(r2 + eps) for r2 in R2])
    else:
        R = sqrtm(R2 + eps)
    # (B, 4)
    ...
    # (B,)
    if rhos.ndim == 3:
        C = np.maximum(0.0, lmbda[:, 3] - lmbda[:, 0:3].sum(axis=1))
    else:
        C = np.max([0,0, lmbda[3] - lmbda[0:3].sum()])
    # Compute Entanglement of formation
    # (B,)
    x = 0.5 * (1.0 + np.sqrt(1.0 - C ** 2))
    eps = np.finfo(rhos.dtype).eps
    # (B,)
    Sent_form = -(x * np.log(x+eps) + (1-x) * np.log(1-x+eps))
    return Sent_form