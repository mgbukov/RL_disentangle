...
class PPOAgent(BaseAgent):
    """Proximal Policy Optimization agent implementation of a reinforcement
    learning agent.
    The agent uses the proximal policy gradient update to improve its policy.
    ...
    def _train(self, num_iter, steps, learning_rate, lr_decay=1.0, clip_grad=10.0, reg=0.0,
        ...
        running_avg_k, running_avg_KL = 0., 0.
        ...
        for i in tqdm(range(num_iter)):
            ...
            # Compute the log probs using the old parameters.
            with torch.no_grad():
            ...
            for k in range(K):
                ...
                loss_clip = torch.sum(mask * torch.min(ro * q_values, clip_adv)) / torch.sum(mask)
            ...
            running_avg_k = 0.99 * running_avg_k + 0.01 * (k+1)
            running_avg_KL = 0.99 * running_avg_KL + 0.01 * (KL.item())
            ...
            self.train_history[i].update({
                ...
                "PPO_avg_num_updates" : running_avg_k,
                "PPO_avg_KL_div"      : running_avg_KL,
